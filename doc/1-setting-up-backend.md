...
* **GPU VRAM:** * **Main LLM (`vllm-coder-mid`):** Minimum $2 \times 24\text{GB}$ GPUs (e.g., $2 \times \text{RTX 3090/4090}$ or enterprise equivalents) to split the model weight overhead via `--tensor-parallel-size 2`.
...
| `./anvil build` | Build Stack | Force build/rebuild of custom Dockerfile images to apply dependency or configuration updates. |
| `./anvil status` | View Status | Verifies active engine containers and performs real-time API health connectivity assessments. |
