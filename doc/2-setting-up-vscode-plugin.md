...
`./anvil setup-repo /path/to/your/target-repo`. `.roo/mcp.json` is regenerated
wholesale, so the Oxylabs entry simply appears, and `.env` only ever gains keys — an
existing `ANTHROPIC_API_KEY` or `GITHUB_TOKEN` survives untouched.
