...
`./anvil setup-repo /path/to/your/target-repo`. `.roo/mcp.json` is **merged**
rather than rewritten: servers you added by hand are preserved untouched (in
position), servers Anvil owns — github, git, oxylabs, package-registry — are
refreshed, and nothing is ever removed. The Oxylabs entry simply appears, and
